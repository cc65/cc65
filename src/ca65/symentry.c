...
/* Define a new symbol */
{
    ...
    if (S->Flags & SF_IMPORT) {
        ...
        Error ("Symbol `%s' is already an import", GetSymNameStr (S));
    ...
    if ((Flags & SF_VAR) != 0 && (S->Flags & (SF_EXPORT | SF_GLOBAL))) {
        ...
        Error ("Var symbol `%s' cannot be an export or global symbol",
               GetSymNameStr (S));
        return;
    }
    if (S->Flags & SF_DEFINED) {
        ...
        if ((S->Flags & SF_VAR) == 0) {
            Error ("Symbol `%s' is already defined", GetSymNameStr (S));
            if (CollCount (&S->DefLines) > 0) {
                PNotification (GetSourcePos (CollAt(&S->DefLines, 0)),
            ...
            if ((Flags & SF_VAR) == 0) {
                Error ("Symbol `%s' is already different kind", GetSymNameStr (S));
                return;
            }
    ...
    if (S->Flags & SF_EXPORT) {
        if (S->ExportSize == ADDR_SIZE_DEFAULT) {
            ...
            Warning (1, "Symbol `%s' is %s but exported %s",
                     GetSymNameStr (S), AddrSizeToStr (S->AddrSize),
...
/* Mark the given symbol as an imported symbol */
{
    ...
    if (S->Flags & SF_EXPORT) {
        ...
        Error ("Cannot import exported symbol `%s'", GetSymNameStr (S));
        return;
    }
    ...
    */
    if (S->Flags & SF_IMPORT) {
        if ((Flags & SF_FORCED) != (S->Flags & SF_FORCED)) {
            Error ("Redeclaration mismatch for symbol `%s'", GetSymNameStr (S));
        }
        if (AddrSize != S->AddrSize) {
            Error ("Address size mismatch for symbol `%s'", GetSymNameStr (S));
        }
    }
    if (S->Flags & SF_GLOBAL) {
        S->Flags &= ~SF_GLOBAL;
...
/* Mark the given symbol as an exported symbol */
{
    ...
    if (S->Flags & SF_VAR) {
        ...
        Error ("Var symbol `%s' cannot be exported", GetSymNameStr (S));
        return;
    }
...
*/
{
    if (S->Flags & SF_VAR) {
        ...
        Error ("Var symbol `%s' cannot be made global", GetSymNameStr (S));
        return;
    }

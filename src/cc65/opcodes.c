...
const OPCDesc OPCTable[OP65_COUNT] = {
    ...
    {   OP65_ASL,                               /* opcode */
        "asl",                                  /* mnemonic */
        0,                                      /* size (why 0?) */
        REG_NONE,                               /* use (why none?) */
        REG_NONE,                               /* chg (why none?) */
...
/* Return the size of the given instruction */
{
    ...
    switch (AM) {
        ...
        default:
            Internal ("Invalid addressing mode in %s: 0x%x", __FUNCTION__, AM);
            return 0;
    }
}
...
*/
{
    switch (OPC) {
        ...
        case OP65_JMP:
	  {
	    if((CPUIsets[CPU] & CPU_ISET_65SC02) || CPU == CPU_C39_EMUL || CPU == CPU_C39_NATIVE)
	      return OP65_BRA;
	    return OP65_JMP;
	  }
        default:

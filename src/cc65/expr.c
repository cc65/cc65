...
*/
{
    ...
    */
    return CurTok.Tok == TOK_LPAREN &&
           (TokIsType(&NextTok) || TokIsTypeQual(&NextTok) ||
            (NextTok.Tok == TOK_IDENT && (Entry = FindSym(NextTok.Ident)) != 0 && SymIsTypeDef(Entry)));
}
...
/* Do deferred operations such as post-inc/dec at sequence points */
{
    ...
    if ((StmtFlags & SQP_KEEP_TEST) != 0 || ((Flags & SQP_KEEP_TEST) != 0 && ED_NeedsTest(Expr))) {
...
/* Perform a function call. */
{
    ...
    if (IsFuncPtr) {
        ...
        ** parameters.
        */
        IsFastcall = (Func->ParamCount > 0 || (Func->Flags & FD_EMPTY) != 0) && IsFastcallFunc(Expr->Type + 1);
        ...
        */
        ...
        IsFastcall = (Func->ParamCount > 0 || (Func->Flags & FD_EMPTY) != 0) && IsFastcallFunc(Expr->Type);
    }
        ...
        if (Sym) {
            ...
            */
            if (IsTypeArray(E->Type)) {
                ...
                ** main().
                */
                if (IS_Get(&Standard) == STD_CC65 && strcmp(Sym->Name, "main") == 0) {
...
/* Process struct/union field after . or ->. */
{
    ...
    } else if (ED_IsRVal(Expr) && ED_IsLocPrimary(Expr) && Expr->Type == GetStructReplacementType(Expr->Type)) {
        ...
        if (SymIsBitField(&Field)) {
            ...
            */
            if (BitOffs + Field.Type->A.B.Width != FieldSize * CHAR_BITS) {
                g_and(CF_INT | CF_UNSIGNED | CF_CONST, (0x0001U << Field.Type->A.B.Width) - 1U);
            }
        } else {
            g_asr(Flags, BitOffs);
        }
...
/* Handle compound types (structs and arrays) */
{
    ...
    /* Check for a rhs */
    while (CurTok.Tok == TOK_INC || CurTok.Tok == TOK_DEC || CurTok.Tok == TOK_LBRACK || CurTok.Tok == TOK_LPAREN ||
...
/* Handle unary -/+ and ~ */
{
    ...
    if (ED_IsConstAbs(Expr)) {
        ...
        if (IsClassFloat(Expr->Type)) {
            switch (Tok) {
            case TOK_MINUS:
                Expr->V.FVal = FP_D_Sub(FP_D_Make(0.0), Expr->V.FVal);
                break;
            case TOK_PLUS:
                break;
            case TOK_COMP:
                Error("Unary ~ operator not valid for floating point constant");
                break;
            default:
                Internal("Unexpected token: %d", Tok);
            }
        } else {
...
/* Helper function for the compare operators */
{
    ...
    while ((Gen = FindGen(CurTok.Tok, Ops)) != 0) {
        ...
        if (!IsRelationType(Expr->Type) || !IsRelationType(Expr2.Type)) {
            ...
            TypeCompatibilityDiagnostic(Expr->Type, Expr2.Type, 1, "Comparing types '%s' with '%s' is invalid");
        ...
        if (IsClassInt(Expr->Type)) {
            if (!IsClassInt(Expr2.Type) && !ED_IsNullPtr(Expr)) {
                if (IsClassPtr(Expr2.Type)) {
                    TypeCompatibilityDiagnostic(
                        Expr->Type, PtrConversion(Expr2.Type), 0, "Comparing integer '%s' with pointer '%s'");
                } else {
                    TypeCompatibilityDiagnostic(Expr->Type, Expr2.Type, 1, "Comparing types '%s' with '%s' is invalid");
                }
            }
            ...
            if (IsClassPtr(Expr2.Type)) {
                ...
                if (TypeCmp(Expr->Type, Expr2.Type).C < TC_VOID_PTR) {
                    ...
                    TypeCompatibilityDiagnostic(PtrConversion(Expr->Type),
                                                PtrConversion(Expr2.Type),
                                                0,
                ...
                if (IsClassInt(Expr2.Type)) {
                    TypeCompatibilityDiagnostic(
                        PtrConversion(Expr->Type), Expr2.Type, 0, "Comparing pointer type '%s' with integer type '%s'");
                } else {
                    TypeCompatibilityDiagnostic(Expr->Type, Expr2.Type, 1, "Comparing types '%s' with '%s' is invalid");
                }
            }
        ...
        if ((ED_IsEntityAddr(Expr) && ED_IsNullPtr(&Expr2)) || (ED_IsNullPtr(Expr) && ED_IsEntityAddr(&Expr2))) {
        ...
        } else if (ED_IsEntityAddr(Expr) && ED_IsEntityAddr(&Expr2) && Expr->Sym == Expr2.Sym) {
            ...
            */
            ...
            if (IsClassInt(Expr->Type) && IsSignSigned(Expr->Type) && IsClassInt(Expr2.Type) &&
                IsSignSigned(Expr2.Type)) {
...
{
    static const GenDesc hie9_ops[] = {{TOK_STAR, GEN_NOPUSH | GEN_COMM, g_mul},
    ...
    if (ED_IsQuasiConst(Expr)) {
        ...
        if (ED_IsQuasiConst(&Expr2)) {
            ...
            if (!AddDone) {
                ...
                if (ED_IsAbs(&Expr2) && (ED_IsAbs(Expr) || lscale == 1)) {
                    if (IsClassInt(lhst) && IsClassInt(rhst)) {
                        Expr->Type = ArithmeticConvert(Expr->Type, Expr2.Type);
                    }
                    Expr->IVal = Expr->IVal * lscale + Expr2.IVal * rscale;
                    AddDone = 1;
                } else if (ED_IsAbs(Expr) && (ED_IsAbs(&Expr2) || rscale == 1)) {
                    if (IsClassInt(lhst) && IsClassInt(rhst)) {
            ...
            */
            ...
            /* Generate the code for the add */
            if (!AddDone) {
                if (ED_IsAbs(Expr) && Expr->IVal >= 0 && Expr->IVal * lscale < 256) {
                    /* Numeric constant */
            ...
            if (!AddDone) {
                if (ED_IsLocQuasiConst(&Expr2) && !IsTypeBitField(Expr2.Type) && rscale == 1 &&
...
*/
{
    ...
    if (IsClassPtr(lhst)) {
        ...
        if (rscale == 0 && (IsClassPtr(rhst) || IsClassInt(rhst))) {
            TypeCompatibilityDiagnostic(lhst, rhst, 1, "Invalid pointer types in subtraction: '%s' and '%s'");
            /* Avoid further errors */
            rscale = 1;
        }
    ...
    */
    if (IsClassPtr(lhst) && IsClassPtr(rhst)) {
        ...
        if (Cmp.C < TC_STRICT_COMPATIBLE) {
            TypeCompatibilityDiagnostic(lhst, rhst, 1, "Incompatible pointer types in subtraction: '%s' and '%s'");
        }
...
{
    static const GenDesc hie6_ops[] = {{TOK_LT, GEN_NOPUSH | GEN_NOFUNC, g_lt},
...
/* Handle == and != */
{
    static const GenDesc hie5_ops[] = {{TOK_EQ, GEN_NOPUSH, g_eq}, {TOK_NE, GEN_NOPUSH, g_ne}, {TOK_INVALID, 0, 0}};
...
/* Handle & (bitwise and) */
{
    static const GenDesc hie4_ops[] = {{TOK_AND, GEN_NOPUSH | GEN_COMM, g_and}, {TOK_INVALID, 0, 0}};
    int UsedGen;
...
{
    static const GenDesc hie3_ops[] = {{TOK_XOR, GEN_NOPUSH | GEN_COMM, g_xor}, {TOK_INVALID, 0, 0}};
    int UsedGen;
...
/* Handle | (bitwise or) */
{
    static const GenDesc hie2_ops[] = {{TOK_OR, GEN_NOPUSH | GEN_COMM, g_or}, {TOK_INVALID, 0, 0}};
    int UsedGen;
...
*/
{
    ...
    if (CurTok.Tok == TOK_BOOL_AND) {
        ...
        /* Convert to bool */
        if ((ED_IsConstAbs(Expr) && Expr->IVal != 0) || ED_IsAddrExpr(Expr)) {
            ED_MakeConstBool(Expr, 1);
        } else {
            Expr->Type = type_bool;
        }
...
/* Parse the ternary operator */
{
    ...
    if (CurTok.Tok == TOK_QUEST) {
        ...
        */
        ...
        if (!IsTypeVoid(Expr2.Type) && ED_YetToLoad(&Expr2) && (!ConstantCond || !ED_IsConst(&Expr2))) {
        ...
        */
        ...
        if (!IsTypeVoid(Expr3.Type) && ED_YetToLoad(&Expr3) && (!ConstantCond || !ED_IsConst(&Expr3))) {
            /* Load it into the primary */
            ...
            */
            if (IsTypeVoid(Indirect(Expr2.Type))) {
                ...
                if (TypeCmp(Expr2.Type, Expr3.Type).C < TC_VOID_PTR) {
                    TypeCompatibilityDiagnostic(
                        Expr2.Type, Expr3.Type, 1, "Incompatible pointer types in ternary: '%s' and '%s'");
                    /* Avoid further errors */
            ...
            if (IsClassStruct(Expr2.Type) && IsClassStruct(Expr3.Type) &&
                ...
                TypeCompatibilityDiagnostic(Expr2.Type, Expr3.Type, 1, "Incompatible types in ternary '%s' with '%s'");
...
/* Parse comma operator. */
{
    ...
    while (CurTok.Tok == TOK_COMMA) {
        ...
        */
        GetCodePos(&End);
        if (CodeRangeIsEmpty(&Start, &End) || (Expr->Flags & E_SIDE_EFFECTS) == 0) {

            if (!ED_MayHaveNoEffect(Expr) && IS_Get(&WarnNoEffect) && PrevErrorCount == ErrorCount) {
